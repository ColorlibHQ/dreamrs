...
function dreamrs_pagination( $args = array() ) {
    ...
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'dreamrs_pagination_defaults', $defaults )
    );
    ...
    if ( $previous && (1 != $page) )
        $echo .= '<li class="previous pgn__prev"><a href="' . esc_attr( $previous ) . '" title="' . esc_html__( 'previous', 'dreamrs' ) . '">' . esc_html__( 'Previous', 'dreamrs' ) . '</a></li>';
    ...
    if ($next && ($count != $page) )
        $echo .= '<li class="next pgn__next"><a href="' . esc_attr( $next ) . '" title="' . esc_html__( 'next', 'dreamrs') . '">' . esc_html__( 'Next', 'dreamrs') . '</a></li>';
